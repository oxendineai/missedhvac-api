...
If the question isn't about HVAC, politely redirect to HVAC topics. For emergencies, mention calling (555) 987-6643.`
        },
        {
          role: "user",
          content: userMessage
  ...
  } catch (_error) { // ← ADDED UNDERSCORE to fix TypeScript error
    console.error('Chat API error:', _error);
    
    return NextResponse.json({
      role: 'assistant',
      content: 'I apologize, but I\'m having trouble connecting right now. For immediate assistance with your HVAC needs, please call (555) 987-6643.'
    }, {
      status: 500,
      headers: {
        'Content-Type': 'application/json',
